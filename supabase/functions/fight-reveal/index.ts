...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Spend 2 tokens
    const newBalance = await spendTokens(supabaseAdmin, user.id, 2, 'FIGHT_REVEAL');
    if (newBalance === null) {
      return new Response(
    ...
    console.error('fight-reveal error:', err);
    return new Response(
      JSON.stringify({ error: '결과 공개 중 오류가 발생했습니다.' }),
